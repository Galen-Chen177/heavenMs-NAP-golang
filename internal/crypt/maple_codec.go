...
func NewMapleEncoder(cypher *MapleCypher) *MapleEncoder {
	return &MapleEncoder{
		cypher: cypher,
	}
}
...
func NewMapleDecoder(cypher *MapleCypher) *MapleDecoder {
	return &MapleDecoder{
		cypher: cypher,
	}
}
...
func (e *MapleDecoder) Decode(input io.Reader, output io.Writer) error {
	...
	for {
		...
		if reader.Buffered() >= packetLength && packetLength != -1 {
			...
			// 解密出内容数据
			MapleDecrypt(decryptedPacket)

			_, err = output.Write(decryptedPacket)

			// 如果开启了被数据调试并且显示包内容
			if config.Cfg.App.Logger.Packet.Debug && config.Cfg.App.Logger.Packet.Display {
...
func NewMapleCodec(encodeCypher *MapleCypher, decodeCypher *MapleCypher) *MapleCodec {
